...
/*
* Replaced: mysql2, dotenv, validateEnvironment, db pool, initDatabase
* Now uses JSON files on disk for persistence — no SQL setup needed.
* The db object exposes the same .execute() interface so all existing
* server.js and API code continues to work without changes.
*/
...
// Auto-generate JWT_SECRET if not set (works without a .env file)
if (!process.env.JWT_SECRET) {
    process.env.JWT_SECRET = 'ecocircuit-local-secret-' + require('os').hostname();
}
if (!process.env.JWT_EXPIRES_IN) process.env.JWT_EXPIRES_IN = '8h';
if (!process.env.INSTITUTIONAL_DOMAINS) process.env.INSTITUTIONAL_DOMAINS = 'national-u.edu.ph';

function validateEnvironment() {
    console.log('[EcoCircuit] Running in JSON-file mode — no SQL setup required.');
}
validateEnvironment();
...
const VALID_DEVICE_TYPES = Object.freeze([
    'laptop', 'smartphone', 'desktop', 'tablet', 'peripheral', 'other',
]);
...
// ─────────────────────────────────────────────────────────────────────────────
// JSON FILE DATABASE
// Stores all data in /data/ folder beside the server script.
// Exposes db.execute(sql, params) with the same interface as mysql2
// so all existing API code works without changes.
// ─────────────────────────────────────────────────────────────────────────────

const DATA_DIR = path.join(
    process.execPath.includes('node') ? process.cwd() : path.dirname(process.execPath),
    'data'
);

const SEED_USERS = [
    { id: 1, email: 'mocklastnamexyz@national-u.edu.ph', name: 'MockName',       password_hash: null, user_type: 'institutional', role: 'student',  is_easter_egg: 0, plain_pw: 'ecocircuit404',              created_at: new Date().toISOString(), last_login: null },
    { id: 2, email: 'radiosilencelu@national-u.edu.ph',  name: 'MediaHost',      password_hash: null, user_type: 'institutional', role: 'staff',    is_easter_egg: 0, plain_pw: 'greatestFather666',           created_at: new Date().toISOString(), last_login: null },
    { id: 3, email: 'mockdataname@gmail.com',            name: 'MockData',       password_hash: null, user_type: 'general',       role: null,        is_easter_egg: 0, plain_pw: 'mockdatapasswordEcoCircuit',  created_at: new Date().toISOString(), last_login: null },
    { id: 4, email: 'wifies@sfawtde.com',                name: 'TheKingInYellow',password_hash: null, user_type: 'general',       role: null,        is_easter_egg: 1, plain_pw: 'dontturnleftatthecrossroads', created_at: new Date().toISOString(), last_login: null },
];
...
function _tablePath(name) { return path.join(DATA_DIR, `${name}.json`); }

function _readTable(name) {
    const f = _tablePath(name);
    if (!fs.existsSync(f)) return [];
    try { return JSON.parse(fs.readFileSync(f, 'utf8')); } catch { return []; }
}

function _writeTable(name, rows) {
    _ensureDataDir();
    fs.writeFileSync(_tablePath(name), JSON.stringify(rows, null, 2), 'utf8');
}
...
// Seed users table with hashed passwords on first run
async function _seedUsers() {
    _ensureDataDir();
    if (fs.existsSync(_tablePath('users'))) return;

    const seeded = [];
    for (const u of SEED_USERS) {
        const hash = await bcrypt.hash(u.plain_pw, BCRYPT_ROUNDS);
        seeded.push({ ...u, password_hash: hash, plain_pw: undefined });
    }
    _writeTable('users', seeded);

    ['devices', 'activity_logs', 'diagnostic_results'].forEach(t => {
        if (!fs.existsSync(_tablePath(t))) _writeTable(t, []);
    });

    console.log('[EcoCircuit] JSON data files seeded and ready.');
}

// Parses a simple WHERE clause like "col = ? AND col2 = ?"
function _applyWhere(rows, whereStr, params) {
    if (!whereStr) return rows;
    let pi = 0;
    const conditions = whereStr.split(/\bAND\b/i).map(c => c.trim());
    const tests = conditions.map(cond => {
        const m = cond.match(/^(\w+)\s*=\s*(\?|\d+)$/);
        if (!m) return () => true;
        const col = m[1];
        const val = m[2] === '?' ? params[pi++] : Number(m[2]);
        return row => String(row[col] ?? '') === String(val ?? '');
    });
    return rows.filter(row => tests.every(t => t(row)));
}

// Simple SQL parser — handles INSERT, SELECT, UPDATE used by server.js / APIs
function _execute(sql, params = []) {
    ...
    if (/^INSERT/i.test(s)) {
        ...
        if (!tableM || !colM) throw new Error('jsondb: bad INSERT');

        const table = tableM[1];
        const cols  = colM[1].split(',').map(c => c.trim());
        const rows  = _readTable(table);
        const row   = { id: _nextId(rows), created_at: new Date().toISOString() };

        cols.forEach((col, i) => {
            const v = params[i];
            row[col] = (v === 'NOW()' || v === undefined) ? (col.endsWith('_at') ? new Date().toISOString() : null) : v ?? null;
        });
    ...
    // ── SELECT ───────────────────────────────────────────────────────────────
    if (/^SELECT/i.test(s)) {
        const fromM  = s.match(/FROM\s+(\w+)/i);
        if (!fromM) throw new Error('jsondb: bad SELECT');

        let rows = _readTable(fromM[1]);

        // JOIN (diagnostic_results JOIN devices)
        const joinM = s.match(/JOIN\s+(\w+)\s+\w+\s+ON\s+([\w.]+)\s*=\s*([\w.]+)/i);
        if (joinM) {
            const jTable = joinM[1];
            const lCol   = joinM[2].split('.')[1];
            const rCol   = joinM[3].split('.')[1];
            const jRows  = _readTable(jTable);
            rows = rows.map(r => {
                const j = jRows.find(jr => jr[rCol] === r[lCol]);
                if (!j) return null;
                const extra = {};
                Object.entries(j).forEach(([k, v]) => { if (k !== 'id') extra[`${jTable}_${k}`] = v; });
                return { ...r, ...extra };
            }).filter(Boolean);
        }
        ...
        if (whereM) rows = _applyWhere(rows, whereM[1], params);

        // ORDER BY
        const orderM = s.match(/ORDER BY\s+([\w.]+)\s*(DESC|ASC)?/i);
        if (orderM) {
    ...
    if (/^UPDATE/i.test(s)) {
        ...
        if (!tableM) throw new Error('jsondb: bad UPDATE');

        const table    = tableM[1];
        const setParts = setM ? setM[1].split(',').map(p => p.trim()) : [];
        const setCount = setParts.filter(p => p.includes('?')).length;
        const setVals  = params.slice(0, setCount);
        const whereVals = params.slice(setCount);

        const rows = _readTable(table);
        const filterFn = whereM ? (r => _applyWhere([r], whereM[1], whereVals).length > 0) : () => true;
        ...
        for (const row of rows) {
            ...
            for (const part of setParts) {
                ...
                if (!m) continue;
                const col = m[1], expr = m[2].trim();
                if (expr === '?') row[col] = setVals[si++];
                else if (/^NOW\(\)$/i.test(expr)) row[col] = new Date().toISOString();
    ...
    if (/^SHOW/i.test(s)) return [[], []];
    throw new Error(`jsondb: unsupported SQL: ${s.slice(0, 80)}`);
}

// Public db object — same interface as mysql2 pool
const db = {
    execute: (sql, params = []) => Promise.resolve(_execute(sql, params)),
...
async function initDatabase() {
    ...
    conn.release();
    console.log('[EcoCircuit] JSON database ready at: ' + DATA_DIR);
}

// ─────────────────────────────────────────────────────────────────────────────
// AUTH SERVICE — same interface, now uses JSON db + real bcrypt + real JWT
// ─────────────────────────────────────────────────────────────────────────────
...
const AuthService = {
    async register({ email, name, password, role }) {
        ...
        if (isInstitutional && (!role || !Object.values(InstitutionalRole).includes(role))) {
            throw new AppError(`Institutional users must provide a valid role: ${Object.values(InstitutionalRole).join(', ')}.`, 400, 'ERR_MISSING_ROLE');
        }

        const [existing] = await db.execute('SELECT id FROM users WHERE email = ? LIMIT 1', [cleanEmail]);
        if (existing.length > 0) throw new AppError('An account with this email already exists.', 409, 'ERR_DUPLICATE_EMAIL');

        const passwordHash = await bcrypt.hash(password, BCRYPT_ROUNDS);

        const [result] = await db.execute(
            `INSERT INTO users (email, name, password_hash, user_type, role, created_at) VALUES (?, ?, ?, ?, ?, NOW())`,
            [cleanEmail, cleanName, passwordHash, userType, role ?? null]
        );

        return { userId: result.insertId, userType, message: `Account created successfully. Welcome to EcoCircuit, ${cleanName}!` };
    },

    async login({ email, password }) {
        const emailCheck = Validator.email(email);
        if (!emailCheck.valid) throw new AppError('Invalid email or password.', 401, 'ERR_INVALID_CREDENTIALS');

        const [rows] = await db.execute('SELECT * FROM users WHERE email = ? LIMIT 1', [emailCheck.value]);
        if (rows.length === 0) throw new AppError('Invalid email or password.', 401, 'ERR_INVALID_CREDENTIALS');

        const user = rows[0];
        const passwordMatch = await bcrypt.compare(password, user.password_hash);
        if (!passwordMatch) throw new AppError('Invalid email or password.', 401, 'ERR_INVALID_CREDENTIALS');

        // Update last_login
        await db.execute('UPDATE users SET last_login = NOW() WHERE email = ?', [emailCheck.value]);

        const token = jwt.sign(
            { userId: user.id, userType: user.user_type, role: user.role },
            JWT_SECRET,
        ...
        return {
            token,
            easterEgg    : user.is_easter_egg === 1,
            isNewAccount : false,
            user         : { userId: user.id, userName: user.name, email: user.email, userType: user.user_type, role: user.role },
        };
    },
    ...
    verifyToken(token) {
        try {
            ...
            throw new AppError('Invalid or expired session. Please log in again.', 401, 'ERR_INVALID_TOKEN');
        }
    },
};
